...
import skrim.install as install
import skrim.utils as utils
...
class TestPackageUnzippedInTempDir(unittest.TestCase):
    ...
    def test_single_wrapper_directory_is_stripped(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with utils.package_unzipped_in_temp_dir(package_path) as (temp_dir, paths):
    ...
    def test_bare_scripts_directory_is_not_stripped(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            targets = install.calculate_heuristic_targets(paths)
    ...
    def test_rar_crc_mismatch_is_tolerated_once_all_bytes_are_written(self):
        ...
        entry = FakeRarEntry('ok.bsa', size=6, blocks=[b'abc', b'def', utils.libarchive.exception.ArchiveError('File CRC error')])

        with tempfile.TemporaryDirectory() as tmp:
            package_path = pathlib.Path(tmp) / 'package.rar'
            with unittest.mock.patch.object(utils.libarchive, 'file_reader', return_value=contextlib.nullcontext([entry])):
                with utils.package_unzipped_in_temp_dir(package_path) as (temp_dir, paths):
    ...
    def test_rar_crc_mismatch_still_raises_when_bytes_are_actually_short(self):
        entry = FakeRarEntry('bad.bsa', size=100, blocks=[b'abc', utils.libarchive.exception.ArchiveError('File CRC error')])

        with tempfile.TemporaryDirectory() as tmp:
            package_path = pathlib.Path(tmp) / 'package.rar'
            with unittest.mock.patch.object(utils.libarchive, 'file_reader', return_value=contextlib.nullcontext([entry])):
                with self.assertRaises(utils.libarchive.exception.ArchiveError):
                    with utils.package_unzipped_in_temp_dir(package_path):
                        pass
