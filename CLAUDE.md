...
2. Add a section for each mod to both `mock.conf` and `mods.conf`, in the same order in both files. Pick the position based on what the Nexus mod page (or other sources, e.g. the mod's own FOMOD/readme, community load-order guides) recommends — not just appended at the end.
